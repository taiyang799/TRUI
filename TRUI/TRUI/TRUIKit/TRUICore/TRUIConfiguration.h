...
/// 所有配置表都应该实现的protocol
@protocol TRUIConfigurationTemplateProtocol <NSObject>
@required
...
/// 当返回YES时，启动App的时候TRUIConfiguration会自动应用这份配置表。但启动App时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 维护项目全局 UI 配置的单例，通过业务项目自己的 TRUIConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 TRUIConfigurationMacros.h 文件里的宏来使用这些值。
 */
@interface TRUIConfiguration : NSObject
...
@property(nonatomic, assign) CGFloat            windowLevelTRUIAlertView;
...
@property(nonatomic, assign) BOOL               shouldPrintTRUIWarnLogToConsole;
@property(nonatomic, assign) BOOL               sendAnalyticsToTRUITeam;
...
@interface UITabBarItem (TRUIConfiguration)
- (void)trui_updateTintColorForiOS12AndEarlier:(nullable UIColor *)tintColor;
@end
