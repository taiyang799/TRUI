...
@interface TRUIPropertyDescriptor : NSObject
...
#pragma mark - Method
CG_INLINE BOOL
HasOverrideSuperclassMethod(Class trargetClass, SEL targetSelector)
{
    Method method = class_getInstanceMethod(trargetClass, targetSelector);
    if (!method) {
        return NO;
    }
    
    Method methodOfSuperClass = class_getInstanceMethod(class_getSuperclass(trargetClass), targetSelector);
...
/**
 如果fromClass里存在originSelector，则这个函数会将fromClass里的originSelector与toClass里的newSelector交换实现。
 如果fromClass里不存在originSelector，则这个函数为fromClass增加方法originSelector，并且该方法会使用toClass的newSelector方法的实现，而toClass的newSelector方法的实现则会被替换为空内容
 @warning 注意如果fromClass里的originSelector是继承自父类并且fromClass也没有重写这个方法，这会导致实际上被替换的是父类，然后父类及父类的所有子类（也即fromClass的兄弟类）也受影响，因此使用时请谨记这一点。因此建议使用OverrideImplementation 系列方法去替换，尽量避免使用ExchangeImplementations。
 @param _fromClass 要被替换的class，不能为空
...
OverrideImplementation(Class targetClass, SEL targetSelector, id(^implementationBlock)(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)))
{
    ...
    if (hasOverride) {
        ...
        const char *typeEncoding = method_getTypeEncoding(originMethod) ?: [targetClass instanceMethodSignatureForSelector:targetSelector].trui_typeEncoding;
 ...
 */
#define _TRUITypeEncodingDetectorGenerator(_TypeInFunctionName, _typeForEncode) \
...
_TRUITypeEncodingDetectorGenerator(Char, char)
_TRUITypeEncodingDetectorGenerator(Int, int)
_TRUITypeEncodingDetectorGenerator(Short, short)
_TRUITypeEncodingDetectorGenerator(Long, long)
_TRUITypeEncodingDetectorGenerator(LongLong, long long)
_TRUITypeEncodingDetectorGenerator(NSInteger, NSInteger)
_TRUITypeEncodingDetectorGenerator(UnsignedChar, unsigned char)
_TRUITypeEncodingDetectorGenerator(UnsignedInt, unsigned int)
_TRUITypeEncodingDetectorGenerator(UnsignedShort, unsigned short)
_TRUITypeEncodingDetectorGenerator(UnsignedLong, unsigned long)
_TRUITypeEncodingDetectorGenerator(UnsignedLongLong, unsigned long long)
_TRUITypeEncodingDetectorGenerator(NSUInteger, NSUInteger)
_TRUITypeEncodingDetectorGenerator(Float, float)
_TRUITypeEncodingDetectorGenerator(Double, double)
_TRUITypeEncodingDetectorGenerator(CGFloat, CGFloat)
_TRUITypeEncodingDetectorGenerator(BOOL, BOOL)
_TRUITypeEncodingDetectorGenerator(Void, void)
_TRUITypeEncodingDetectorGenerator(Character, char *)
_TRUITypeEncodingDetectorGenerator(Object, id)
_TRUITypeEncodingDetectorGenerator(Class, Class)
_TRUITypeEncodingDetectorGenerator(Selector, SEL)
...
#define _TRUIGetIvarValueGenerator(_TypeInFunctionName, _typeForEncode) \
...
_TRUIGetIvarValueGenerator(Char, char)
_TRUIGetIvarValueGenerator(Int, int)
_TRUIGetIvarValueGenerator(Short, short)
_TRUIGetIvarValueGenerator(Long, long)
_TRUIGetIvarValueGenerator(LongLong, long long)
_TRUIGetIvarValueGenerator(UnsignedChar, unsigned char)
_TRUIGetIvarValueGenerator(UnsignedInt, unsigned int)
_TRUIGetIvarValueGenerator(UnsignedShort, unsigned short)
_TRUIGetIvarValueGenerator(UnsignedLong, unsigned long)
_TRUIGetIvarValueGenerator(UnsignedLongLong, unsigned long long)
_TRUIGetIvarValueGenerator(Float, float)
_TRUIGetIvarValueGenerator(Double, double)
_TRUIGetIvarValueGenerator(BOOL, BOOL)
_TRUIGetIvarValueGenerator(Character, char *)
_TRUIGetIvarValueGenerator(Selector, SEL)
...
/**
 ...
 int count = trui_getProjectClassList(&classes);
 Class class = (__bridge Class)classes[0];
 @endcode
 */
FOUNDATION_EXPORT int trui_getProjectClassList(classref_t **classes);
